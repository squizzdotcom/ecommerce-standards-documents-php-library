<?php
	...
	*}
	*/
	class ESDocumentCustomerInvoice extends ESDocument implements \JsonSerializable
	{
		/** @var ESDRecordCustomerInvoice[] List of customer invoice records*/
		public $dataRecords = array();
		
		/**
		* Constructor
		*  @param resultStatus 				int						status of obtaining the customer invoice data
		*  @param message 					string					message to accompany the result status
		*  @param customerInvoiceRecords	ESDRecordOrderSale[]	list of customer invoice records
		*  @param configs 					array					A list of key value pairs that contain additional information about the document.
		*/
		public function __construct($resultStatus = 0, $message = "", $customerInvoiceRecords = array(), $configs = array())
		{
			$this->resultStatus = $resultStatus;
			$this->message = $message;
			$this->dataRecords = $customerInvoiceRecords;
			$this->configs = $configs;
			if ($customerInvoiceRecords != null)
			{
				$this->totalDataRecords = count($customerInvoiceRecords);
			}
		}
